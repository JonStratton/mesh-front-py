...
if [ $OLSR = 1 ]; then
   ...
   controlled_system_files="$controlled_system_files /etc/olsrd/olsrd.conf /etc/olsrd/olsrd.key /etc/default/olsrd"
fi
...
if [ $CJDNS ]; then; install_cjdns; fi
if [ $OLSR ]; then; install_olsrd; fi
...
sudo /usr/bin/cjdroute > /etc/cjdroute.conf
cd ..
rm -rf cjdns-master
...
if [ $CJDNS ]; then; uninstall_cjdns; fi
if [ $OLSR ]; then; uninstall_olsrd; fi
